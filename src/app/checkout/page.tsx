...
const Page: React.FC = () => {
  ...
  const fetchCourses = async (email: string) => {
   ...
   const calculatedTotal = uniqueCourses.reduce((sum: number, item: Course) => {
  const price = Number(item.course_price);
...
setTotal(calculatedTotal);
    ...
    <div className="min-h-screen pt-20 bg-gradient-to-br from-[#F4F6F8] to-[#E5E7EB] px-4 md:px-10 pb-10">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
          <button
            onClick={() => router.push("/")}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg font-semibold transition-all duration-200 hover:scale-105 hover:shadow-md cursor-pointer"
            style={{
  ...
  style={{
    backgroundColor: BRAND_COLORS.deepRed, // Changed from teal to deepRed
    color: BRAND_COLORS.white,
  }}
>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          {courses.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <button
                onClick={() => router.push("/")}
                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg font-semibold transition-all hover:scale-105 cursor-pointer"
                style={{
            ...
            <>
              ...
              <div className="divide-y">
                {courses.map((course) => {
                  // Fix: Properly parse price
                  let price = 0;
                  if (typeof course.course_price === 'string') {
                    price = parseFloat(course.course_price);
                  } else {
...
>
  Rs. {Number(total).toLocaleString()}
                ...
                {/* Action Buttons - Made Complete Enrollment button RED with cursor pointer */}
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <button
                    onClick={() => router.push("/")}
                    className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg font-semibold border-2 transition-all hover:bg-gray-50 cursor-pointer"
                    style={{
                  ...
                  <button
                    onClick={handleEnroll}
                    className="flex-1 inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg font-semibold transition-all hover:scale-105 hover:shadow-lg cursor-pointer"
                    style={{
