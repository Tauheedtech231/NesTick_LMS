...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 2. Get all slides with their files count
    ...
    // 3. Get slide progress for this student from slide_progress table
    ...
    try {
      ...
      console.warn('⚠️ slide_progress table might not exist:', tableError);
    ...
    // 4. Get completed files per slide from content_views table
    ...
    try {
      ...
      console.warn('⚠️ content_views table might not exist:', tableError);
    ...
    // 5. Get overall student progress from student_progress table
    let overallProgress = {
    ...
    const slidesWithProgress = slides.map(slide => {
      ...
      // Use status from slide_progress if available, otherwise determine from overall progress
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        overallProgress // Include overall progress for reference
      }
    });
