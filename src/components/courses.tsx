...
import { 
  HiClock, HiUserGroup, HiAcademicCap, HiCheckCircle,
  HiArrowRight, HiBookOpen, HiShieldCheck, HiOutlineFire as HiWrench,
  HiFire, HiStar, HiSearch, HiX, HiClock as HiRecent,
  HiBadgeCheck, HiCog, HiBriefcase, HiMail, HiCheck,
  HiExclamation, HiTrash, HiShoppingBag, HiCurrencyRupee,
  HiUser, HiTag, HiGift, HiEye, HiOutlineGift, HiInformationCircle
} from "react-icons/hi";
...
// ✅ FIX 1: Memoize helper functions outside component
...
// ✅ FIX 2: Memoize Email Popup component
const EmailPopup = memo(({ isOpen, onClose, onConfirm, courseTitle, savedEmail }: EmailPopupProps) => {
...
// ✅ FIX 3: Extract CourseCard as memoized component
const CourseCard = memo(({ 
  ...
  onAddToCart: (item: DisplayItem) => void;
  onRemoveFromCart: (courseId: string, cartId?: string) => void;
  ...
  const handleCartClick = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (isInCart) {
      const cartItem = JSON.parse(localStorage.getItem('tempCartItem') || '{}');
      onRemoveFromCart(course.id, cartItem.id);
    } else {
      onAddToCart(item);
    }
  }, [isInCart, course.id, item, onAddToCart, onRemoveFromCart]);

  // ✅ FIX 4: Memoize fadeIn animation variant
  const fadeInVariant = useMemo(() => ({
    initial: { y: 30, opacity: 0 },
    animate: { y: 0, opacity: 1 },
    transition: { duration: 0.5, ease: [0.4, 0, 0.2, 1] as Easing, delay: Math.min(index * 0.05, 0.5) }
  }), [index]);
        ...
        <div className="relative h-48 sm:h-56 overflow-hidden bg-gray-100 flex-shrink-0">
          {course.image && !imageError ? (
            <img
              ...
              loading="lazy" // ✅ FIX 5: Native lazy loading
              decoding="async" // ✅ FIX 6: Async decoding
            />
          ) : (
            ...
            <div className="flex gap-2">
              ...
              {isInCart ? (
                ...
                <button
                  ...
                  style={{
                    backgroundColor: hoveredId === item.id ? '#1E3A8A' : '#B11217',
                  }}
                >
                  {isLoading ? (
...
// ✅ FIX 7: Extract BundleCard as memoized component
...
export default function CoursesPage() {
  ...
  // ✅ FIX 8: Memoize expensive icon mapping
  ...
  // ✅ FIX 10: Memoize fetchFreshData to prevent recreation
  ...
  }, [fetchFreshData, getIconComponent]);

  useEffect(() => {
    loadData();
  }, [loadData]);

  // ✅ FIX 11: Memoize cart fetch function
  ...
  }, [userEmail, initialLoadComplete, fetchCartCount]);

  // ✅ FIX 12: Memoize displayItems calculation (prevents unnecessary recomputations)
  const displayItems = useMemo(() => {
    ...
    return items;
  }, [allCourses, allBundles, inCartStatus, formatCurrency]);

  // ✅ FIX 13: Memoize cart total calculation
  ...
  // ✅ FIX 14: Memoize bundle savings info calculation
  const getBundleSavingsInfo = useCallback(() => {
  ...
  const bundleGroups = useMemo(() => getBundleSavingsInfo(), [getBundleSavingsInfo]);

  const handleImageError = useCallback((itemId: string) => {
    setImageErrors(prev => ({ ...prev, [itemId]: true }));
  }, []);

  const handleViewBundleDetails = useCallback((bundleId: string) => {
    router.push(`/courses/bundles/${bundleId}`);
  }, [router]);

  // ✅ FIX 15: Memoize cart handlers
  const addToCart = useCallback(async (course: Course, email: string) => {
  ...
  const addBundleToCart = useCallback(async (bundle: Bundle, email: string) => {
  ...
  }, [fetchCartCount, formatCurrency, inCartStatus]);

  const handleAddToCartClick = useCallback((item: DisplayItem) => {
    if (userEmail) {
      if (item.type === 'course') {
  ...
  }, [userEmail, addToCart, addBundleToCart]);
  ...
  }, [selectedItem, addToCart, addBundleToCart]);

  const handleRemoveFromCart = useCallback(async (courseId: string, cartId?: string) => {
  ...
  }, [userEmail, fetchCartCount]);

  const handleProceedToEnrollment = useCallback(() => {
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-400 mb-2">Added on {new Date(item.added_at).toLocaleDateString()}</p>
                                ...
                                <div className="flex items-center justify-between">
                                  ...
                                  <button
                                    onClick={() => handleRemoveFromCart(item.course_id, item.id)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {displayItems.length > 0 ? (
            displayItems.map((item, index) => {
              if (item.type === 'bundle') {
                const isLoading = cartLoading[`bundle_${item.id}`];
                return (
                  <BundleCard
                    key={`bundle-${item.id}`}
                    item={item}
                ...
                const isInCart = inCartStatus[item.id];
                const isLoading = cartLoading[item.id];
                const isRemoving = removingFromCart === cartItems.find(ci => ci.course_id === item.id)?.id;
                
                // ✅ FIX 16: Store temp cart item for removal
                const cartItem = cartItems.find(ci => ci.course_id === item.id);
                if (cartItem) {
